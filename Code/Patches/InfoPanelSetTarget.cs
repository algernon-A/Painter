...
namespace Repaint
{
    ...
    /// <summary>
    /// Harmony patch for VarsitySportsArenaPanel.OnSetTarget to show the the color field and set it to the current building's settings.
    /// </summary>
    [HarmonyPatch(typeof(VarsitySportsArenaPanel), "OnSetTarget")]
    public static class VarsitySportsArenaPanelPatch
    {
        /// <summary>
        /// Harmony Postfix patch to show color field and set to current building's settings.
        /// </summary>
        /// <param name="__instance">Panel instance</param>
        public static void Postfix(VarsitySportsArenaPanel __instance)
        {
            // Show panel.
            Repaint.instance.IsPanelVisible = true;
            ...
            // Set color to this building's setting.
            Repaint.instance.BuildingID = ((InstanceID)__instance.GetType().GetField("m_InstanceID", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(__instance)).Building;
            Repaint.instance.ColorFields[PanelType.VarsityStadium].selectedColor = Repaint.instance.GetColor();
        }
    }
}
